...
@Weave(originalName = "reactor.netty.http.server.HttpServerRoutes", type = MatchType.Interface)
public class HttpServerRoutes_Instrumentation {
    ...
    private void addURLMapping(Predicate<? super HttpServerRequest> condition, String className){
        try {
            ...
            NewRelicSecurity.getAgent().log(LogLevel.WARNING, String.format(GenericHelper.ERROR_WHILE_GETTING_APP_ENDPOINTS, "NETTY-REACTOR-0.8.0", e.getMessage()), e, this.getClass().getName());
        }
    }
}
