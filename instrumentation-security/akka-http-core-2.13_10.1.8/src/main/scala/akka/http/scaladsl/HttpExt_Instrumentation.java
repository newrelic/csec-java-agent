...
public class HttpExt_Instrumentation {
    ...
    private AbstractOperation preprocessSecurityHook(HttpRequest httpRequest, String methodName) {
        try {
            ...
            try {
                NewRelicSecurity.getAgent().recordExternalConnection(methodURI.getHost(), methodURI.getPort() == -1 ? methodURI.toURL().getDefaultPort() : methodURI.getPort(), uri, null, ExternalConnectionType.HTTP_CONNECTION.name(), AkkaCoreUtils.AKKA_HTTP_CORE_10_0_11);
