...
public class WebClientTest {
    ...
    public void testSend() throws InterruptedException {
        ...
        webClient.get(port, "localhost", "/hi").send().onComplete(reqHandler(latch));
        latch.await();
    ...
    public void testSend1() throws InterruptedException {
        CountDownLatch latch = new CountDownLatch(1);
        HttpRequest<Buffer> request = webClient.request(HttpMethod.GET, port, "localhost", "/hi");
        request.send().onComplete(reqHandler(latch));
        latch.await();
    ...
    public void testSend3() throws InterruptedException {
        CountDownLatch latch = new CountDownLatch(1);
        webClient.request(HttpMethod.GET, port, "localhost", "/hi").sendBuffer(Buffer.buffer("String buffer"))
                .onComplete(reqHandler(latch));
        latch.await();
    ...
    public void testUnknownHost() {
        ...
        SecurityIntrospector introspector = SecurityInstrumentationTestRunner.getIntrospector();
        Assert.assertTrue(introspector.getOperations().isEmpty());
    }
