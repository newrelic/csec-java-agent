    ...
    public static void registerExitOperation(boolean isProcessingAllowed, AbstractOperation operation) {
        try {
            ...
            NewRelicSecurity.getAgent().log(
                    LogLevel.FINEST, String.format(GenericHelper.EXIT_OPERATION_EXCEPTION_MESSAGE, GrpcUtils.GRPC_1_40_0, e.getMessage()), e, GrpcClientUtils.class.getName());
        }
    }
    ...
    public static AbstractOperation preprocessSecurityHook(String uri, Metadata meta, String klass) {
        try {
            ...
            if (e instanceof NewRelicSecurityException) {
                NewRelicSecurity.getAgent().log(LogLevel.WARNING, String.format(GenericHelper.SECURITY_EXCEPTION_MESSAGE, GrpcUtils.GRPC_1_40_0, e.getMessage()), e, GrpcClientUtils.class.getName());
                throw e;
            }
            NewRelicSecurity.getAgent().log(LogLevel.SEVERE, String.format(GenericHelper.REGISTER_OPERATION_EXCEPTION_MESSAGE, GrpcUtils.GRPC_1_40_0, e.getMessage()), e, GrpcClientUtils.class.getName());
            NewRelicSecurity.getAgent().reportIncident(LogLevel.SEVERE, String.format(GenericHelper.REGISTER_OPERATION_EXCEPTION_MESSAGE, GrpcUtils.GRPC_1_40_0, e.getMessage()), e, GrpcClientUtils.class.getName());
        }
        return null;
    }
