...
object ResponseFutureHelper {
    ...
    Future {
      ...
      try {
        ...
        processingResult.onComplete {
         _ => {
           token.linkAndExpire()
           AkkaCoreUtils.postProcessHttpRequest(isLockAquired, stringResponse, response.entity.contentType.toString(), response.status.intValue(), response.status.intValue(), this.getClass.getName, "apply", NewRelic.getAgent.getTransaction.getToken)
         }
        }
  ...
  def wrapResponseSync(httpResponse: HttpResponse, materializer: Materializer) {
    try {
      ...
      AkkaCoreUtils.postProcessHttpRequest(isLockAquired, stringResponse, httpResponse.entity.contentType.toString(), httpResponse.status.intValue(), httpResponse.status.intValue(), this.getClass.getName, "apply", NewRelic.getAgent.getTransaction.getToken())
    } catch {
      case t: NewRelicSecurityException =>
        NewRelicSecurity.getAgent.log(LogLevel.WARNING, String.format(GenericHelper.SECURITY_EXCEPTION_MESSAGE, AkkaCoreUtils.AKKA_HTTP_CORE_10_0_11, t.getMessage), t, classOf[AkkaCoreUtils].getName)
        throw t
