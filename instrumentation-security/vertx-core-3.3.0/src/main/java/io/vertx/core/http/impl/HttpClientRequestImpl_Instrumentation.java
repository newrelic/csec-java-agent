...
public abstract class HttpClientRequestImpl_Instrumentation {
    ...
    public void end(Buffer chunk) {
        ...
        if(isLockAcquired) {
            ...
            operation = preprocessSecurityHook(uri, this.getClass().getName(), VertxClientHelper.METHOD_END);
    ...
    private AbstractOperation preprocessSecurityHook(String url, String className, String methodName) {
        try {
            ...
            if (e instanceof NewRelicSecurityException) {
                NewRelicSecurity.getAgent().log(LogLevel.WARNING, String.format(GenericHelper.SECURITY_EXCEPTION_MESSAGE, VertxClientHelper.VERTX_CORE_3_3_0, e.getMessage()), e, this.getClass().getName());
                throw e;
            }
            NewRelicSecurity.getAgent().log(LogLevel.SEVERE, String.format(GenericHelper.REGISTER_OPERATION_EXCEPTION_MESSAGE, VertxClientHelper.VERTX_CORE_3_3_0, e.getMessage()), e, this.getClass().getName());
            NewRelicSecurity.getAgent().reportIncident(LogLevel.SEVERE, String.format(GenericHelper.REGISTER_OPERATION_EXCEPTION_MESSAGE, VertxClientHelper.VERTX_CORE_3_3_0, e.getMessage()), e, this.getClass().getName());
        }
        return null;
    }
    ...
    private void registerExitOperation(boolean isProcessingAllowed, AbstractOperation operation) {
        try {
            ...
            NewRelicSecurity.getAgent().log(LogLevel.FINEST, String.format(GenericHelper.EXIT_OPERATION_EXCEPTION_MESSAGE, VertxClientHelper.VERTX_CORE_3_3_0, e.getMessage()), e, this.getClass().getName());
        }
    }

    private String getAbsoluteUri(boolean ssl, String host, String relativeUri) {
        return ssl ? "https://" : "http://" + host + relativeUri;
    }
}
