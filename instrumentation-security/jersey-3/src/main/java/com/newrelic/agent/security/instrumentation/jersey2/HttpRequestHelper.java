...
public class HttpRequestHelper {
    ...
    public static void preprocessSecurityHook(ContainerRequest requestContext) {
        try {
            ...
            NewRelicSecurity.getAgent().log(LogLevel.WARNING, String.format(GenericHelper.ERROR_GENERATING_HTTP_REQUEST, JERSEY_3, e.getMessage()), e, HttpRequestHelper.class.getName());
        }
    }
    ...
    public static void postProcessSecurityHook(String className, OutboundMessageContext wrappedMessageContext) {
        try {
            ...
            if (e instanceof NewRelicSecurityException) {
                NewRelicSecurity.getAgent().log(LogLevel.WARNING, String.format(GenericHelper.SECURITY_EXCEPTION_MESSAGE, JERSEY_3, e.getMessage()), e, HttpRequestHelper.class.getName());
                throw e;
            }
            NewRelicSecurity.getAgent().log(LogLevel.SEVERE, String.format(GenericHelper.REGISTER_OPERATION_EXCEPTION_MESSAGE, JERSEY_3, e.getMessage()), e, HttpRequestHelper.class.getName());
            NewRelicSecurity.getAgent().reportIncident(LogLevel.SEVERE, String.format(GenericHelper.REGISTER_OPERATION_EXCEPTION_MESSAGE, JERSEY_3, e.getMessage()), e, HttpRequestHelper.class.getName());
        }
    }
