...
public abstract class URLConnection_Instrumentation {
    ...
    private AbstractOperation preprocessSecurityHook(boolean currentCascadedCall, String callArgs, String protocol, String methodName) {
        try {
            ...
            if (securityMetaData.getRequest().isEmpty() || callArgs == null || callArgs.trim().isEmpty() || currentCascadedCall
            ) {
                return null;
            }
            cascadedCall = true;
...
//            this.addRequestProperty("K2-API-CALLER", "");
            ...
            if(StringUtils.isNotBlank(csecParaentId)){
                this.setRequestProperty(GenericHelper.CSEC_PARENT_ID, csecParaentId);
            }
