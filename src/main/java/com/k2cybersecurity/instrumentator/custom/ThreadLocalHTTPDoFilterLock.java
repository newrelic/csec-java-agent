...
public class ThreadLocalHTTPDoFilterLock {
    ...
    private static ThreadLocal<ThreadLocalHTTPDoFilterLock> instance = new ThreadLocal<ThreadLocalHTTPDoFilterLock>() {
        @Override
        protected ThreadLocalHTTPDoFilterLock initialValue() {
            return new ThreadLocalHTTPDoFilterLock();
        }
    };

    private ThreadLocalHTTPDoFilterLock() {
        lock = new Semaphore(1);
    }

    public static ThreadLocalHTTPDoFilterLock getInstance() {
        return instance.get();
    }
