...
class LoggingInterceptor implements Interceptor {
    ...
    public static final String RECEIVED_RESPONSE_FOR_S_IN_1_FMS_N_S = "Received response for %s in %.1fms%n%s";
    ...
    public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        logger.log(LogLevel.INFO, String.format(SENDING_REQUEST_S_ON_S_N_S,
                request.url(), chain.connection(), request.headers()), LoggingInterceptor.class.getName());

        Response response = chain.proceed(request);

        long t2 = System.nanoTime();
        logger.log(LogLevel.INFO, String.format(RECEIVED_RESPONSE_FOR_S_IN_1_FMS_N_S,
