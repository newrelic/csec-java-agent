...
public class Callbacks {
	...
	public static Class httpAsyncRequestProducer = null;
...
//					System.out.println(String.format("Entry : SSRF : %s : %s", className, methodName));
					...
					if(httpAsyncRequestProducer == null){
						ClassLoader classLoader =  AgentUtils.getInstance().getClassLoaderRecord().get("org.apache.http.nio.protocol.HttpAsyncRequestProducer");
						if(classLoader != null) {
							httpAsyncRequestProducer = classLoader.loadClass("org.apache.http.nio.protocol.HttpAsyncRequestProducer");
						}
					}
...
//						System.out.println(String.format("Entry Value : SSRF : %s : %s : %s", className, methodName, uri.toString()));
					...
					} else if(httpAsyncRequestProducer != null && httpAsyncRequestProducer.isInstance(args[0])){
						// TODO : Async part to be implemented.
					}
				}
			} finally {
