			...
			try {
				...
				if (returnVal != null && args != null && StringUtils.isNotBlank(args[0].toString()) && sourceString
					...
					try {
						...
						&& sourceString.contains("org.brackit.xquery.XQuery")) {
					try {
					...
					if (queryString != null) {
						...
						if(moduleObject!=null) {
							...
							ThreadLocalXQuerySaxonMap.getInstance().create(moduleObject, queryString, className, methodName, exectionId, Instant.now().toEpochMilli());
						}
					}
					} catch (Exception e) {
						e.printStackTrace();
