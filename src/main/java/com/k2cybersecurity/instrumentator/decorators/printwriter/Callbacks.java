			...
			try {
				...
				if (StringUtils.startsWith(methodName, "format")) {
					...
					if (args != null && args.length == 2 && args[0] instanceof String) {
						...
						formatter.format((String) args[0], args[1]);
					} else if (args != null && args.length == 3 && args[0] instanceof Locale) {
						formatter = new Formatter((Appendable) obj, (Locale) args[0]);
						formatter.format((Locale) args[0], (String) args[1], args[2]);
					}
					if (formatter != null) {
						ThreadLocalHttpMap.getInstance().insertToResponseBuffer(formatter.toString());
					} else {
						System.out.println(
